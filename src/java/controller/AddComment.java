    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (session.getAttribute("UserObject") != null) {
                ...
                int rowAffected;
                try {
                    rowAffected = p.AddCommentMethod(p.getCode(), Comment, POSTID);

                    if (rowAffected > 0) {
response.sendRedirect("MyAccount.jsp?innerpage=CourseViewPOSTContents&POSTID=" +POSTID+"&Subject_Code="+Subject_Code+"");
